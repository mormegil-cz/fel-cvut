...
int main(int argc, char* argv[])
{
    ...
    net_send(s, (char *)&head, sizeof(head), (struct sockaddr *)&sin, sizeof(sin));
    net_send(s, argv[3], head.fnamelen, (struct sockaddr *)&sin, sizeof(sin));

	/* Komunikace, cast 2 - precti "vysledek hlavicky" */
	/*net_receive(s, (char *)&hack, sizeof(hack), (struct sockaddr *)&sin, sizeof(sin));
	switch (hack.result) {
		case ACKR_OK: break;
		case ACKR_EXISTS:   {
								int c = '\n';
								...
								if (c == 'N') {
									printf("OK, aborting.");
                                    confirmation = CONF_SKIP;
                                    net_send(s, &confirmation, 1, (struct sockaddr *)&sin, sizeof(sin));
                                    printf("\n");
									terminate(1);
								}
	...
	for(remaining = head.filesize; remaining>0; ) {
		...
		net_send(s, buf, toread, (struct sockaddr *)&sin, sizeof(sin));
